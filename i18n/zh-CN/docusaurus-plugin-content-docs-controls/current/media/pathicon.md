...
id: pathicon
title: PathIcon
description: 一个根据 SVG 路径（流几何）数据渲染矢量图标的控件，常用于按钮、菜单和其他交互控件中。
doc-type: reference
---

import PathIconHouseScreenshot from '/img/controls/pathicon/pathicon-house.png';

[`PathIcon`](/api/avalonia/controls/pathicon) 控件会根据流几何数据渲染矢量图标。你可以用它在应用中显示可缩放、与分辨率无关的图标。[Avalonia Fluent 图标库](https://avaloniaui.github.io/icons.html) 提供了大量现成可用的几何字符串。

由于 `PathIcon` 使用的是矢量路径而不是光栅图像，因此图标在任意尺寸下都能清晰缩放，并且你可以通过修改 `Foreground` 画刷来重新着色。

`PathIcon` 最常见的用法是作为其他控件内容的一部分，例如为 `Button`、`MenuItem` 或 `NavigationViewItem` 添加图标。

## 常用属性

| 属性 | 类型 | 说明 |
|---|---|---|
| `Data` | `Geometry` | 定义图标形状的流几何数据。 |
| `Foreground` | `IBrush` | 用于填充图标的画刷。如果未设置该属性，则会从父控件继承。 |
| `Width` | `double` | 图标渲染后的宽度。 |
| `Height` | `double` | 图标渲染后的高度。 |

## 基本示例

下面的示例会显示一个 200 x 200 像素的商店图标，并使用蓝色填充。几何数据被存储为 `StreamGeometry` 资源，并通过 `StaticResource` 引用。
...
`PathIcon` 常用于为交互控件添加图标。下面的示例展示了它在 `Button` 和 `MenuItem` 中的典型用法。
...
若要在多个位置重用同一个图标，请将 `StreamGeometry` 值存放到应用级或窗口级的资源字典中。
...
之后，你就可以在整个应用中使用 `{StaticResource key_name}` 来引用任意图标。

你可以浏览 [Avalonia Fluent 图标](https://avaloniaui.github.io/icons.html)，查看一个可搜索的现成几何数据图标库。

## 动态更改图标颜色

由于 `PathIcon` 会从父控件继承 `Foreground`，因此你可以通过样式或绑定更改图标颜色，而无需在每个实例上单独设置该属性。

```xml
<Button Foreground="Green">
    <!-- PathIcon 会从 Button 继承绿色 -->
    <PathIcon Data="{StaticResource check_regular}" Width="16" Height="16" />
</Button>
```

如果你想为某个特定图标覆盖继承而来的颜色，可以直接设置 `Foreground`。
...
- [Fluent 图标画廊](https://avaloniaui.github.io/icons.html)
