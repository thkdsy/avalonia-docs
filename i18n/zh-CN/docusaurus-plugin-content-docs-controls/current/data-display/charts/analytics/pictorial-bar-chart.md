...
title: 象形柱状图
description: 使用图标或符号代替普通柱条，在信息图和主题报告中以视觉方式强化主题内容。
doc-type: reference
tags:
  - avalonia pro
---
...
象形柱状图使用可识别的图标或符号代替普通柱条。它们适用于信息图，其中主题内容（例如人物、汽车或建筑）可以通过视觉方式得到强化。

<Image light={chartsCartesianPictorial} maxWidth={400} position="center" cornerRadius="true" alt="象形柱状图，使用重复的图标符号代替普通柱条来表示类别数值。" />

## 使用场景
- **信息图**：创建高参与度的主题报告。
- **人口统计报告**：使用性别或职业图标表示数量。
- **公共数据展示**：通过熟悉的符号让枯燥的统计数据更易理解。
...
## 常用属性 (PictorialBarSeries)

| 属性 | 描述 | 默认值 |
| :--- | :--- | :--- |
| `Geometry` | 图标的 SVG/XAML 路径数据。 | `null` |
| `RenderMode` | `Stretch`（拉伸）、`Clip`（裁剪）或 `Repeat`（重复）。 | `Stretch` |
| `SymbolSize` | 图标的大小（像素）。 | `20.0` |
| `SymbolSpacing` | 重复图标之间的间距。 | `5.0` |
| `Fill` | 用于绘制图标的画笔。 | 取决于主题 |
