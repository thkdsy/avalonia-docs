...
id: togglebutton
title: ToggleButton
description: 一个可在选中和未选中状态之间切换的按钮，并可选支持三态。
doc-type: reference
---

import ToggleButtonMuteScreenshot from '/img/controls/buttons/togglebutton/togglebutton-mute.gif';

[`ToggleButton`](/api/avalonia/controls/primitives/togglebutton) 控件通过样式和伪类来表示一个布尔值：伪类存在表示 true，不存在表示 false。这使你能够为控件在不同伪类状态下创建各种图形化表现形式。

`ToggleButton` 位于 `Avalonia.Controls.Primitives` 命名空间中，并作为 `CheckBox` 及其他切换式控件的基类。
...
| 属性 | 说明 |
| --------------- | --------------------------------------------------------------------------- |
| `IsChecked` | 获取或设置 `ToggleButton` 是否处于选中状态。该值是可空的 `bool`。 |
| `IsThreeState` | 获取或设置控件是否支持三态。 |
| `ClickMode` | 决定 `Click` 事件在何时触发（释放、按下或悬停时）。 |

## 三态行为

默认情况下，`ToggleButton` 会在两种状态之间循环：选中和未选中。当你将 `IsThreeState` 设置为 `true` 时，控件会按顺序在三种状态之间循环：选中、未选中和不确定。

在三态模式下，`IsChecked` 属性是一个可空的 `bool?`：

- `true` 对应选中状态（`:checked` 伪类）。
- `false` 对应未选中状态。
- `null` 对应不确定状态（`:indeterminate` 伪类）。

你可以在 AXAML 样式中使用伪类，为每种状态分别设置样式。

## 示例

### 使用伪类设置样式

此示例展示了一个包含扬声器图标或静音扬声器图标的切换按钮，具体显示哪个图标取决于按钮是否具有 checked 伪类。
...
切换按钮的内容区域包含两个路径图标元素，同一时间只会显示其中一个。路径图标的图形来自一个资源文件，该文件作为包含的样式集在 `App.xaml` 中被引用。这些图标几何数据来自 Avalonia 的 [Fluent icons resource](https://avaloniaui.github.io/icons.html)。

路径图标的可见性由窗口样式控制，这些样式通过 `:checked` 伪类来判断切换按钮是否处于选中状态。当按钮被选中时，`audio-on` 路径图标可见，而 `audio-mute` 路径图标隐藏；反之，当按钮未选中时，`audio-mute` 图标可见，而 `audio-on` 图标隐藏。

### 绑定到视图模型

你可以将 `IsChecked` 属性绑定到视图模型中的 `bool` 或 `bool?` 属性。这是在 MVVM 应用中使用 `ToggleButton` 最常见的方式。
...
当你需要表示不确定状态时，请将 `IsThreeState` 设置为 `true`，并绑定到一个可空的 `bool?` 属性。
